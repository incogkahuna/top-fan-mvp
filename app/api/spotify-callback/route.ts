...
export async function GET(request: NextRequest) {
  try {
    ...
    // Create a simple HTML response instead of redirecting
    let html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    if (error) {
      ...
      html += `
        <div class="success">
            <h2>✅ Spotify Authorization Code Received!</h2>
            <p><strong>Code:</strong> ${code.substring(0, 20)}...</p>
            <p><strong>State:</strong> ${state || 'Not provided'}</p>
            <p><strong>Full URL:</strong> <pre>${request.url}</pre></p>
            <p>🎉 The OAuth flow is working! Next step: Exchange code for access token.</p>
        </div>
      `
    } else {
      html += `
